...
public class SearchUserConversation extends BasicConversation {
	...
	@Override
	protected void showInitialMessage(SlackChannel channel, SlackUser messageSender, SlackSession session, String userInput) throws ConversationException {
		simulateHumanStyleMessageSending(channel, "Ok, what Slack user are you looking for?  Enter a name and I'll find what I can! (Type \"exit\" to cancel)", session);
	}

	@Override
	protected boolean handleResponse(SlackChannel channel, SlackUser messageSender, SlackSession session, String userInput) throws ConversationException {
		if (userInput == null || userInput.isBlank()) {
			throw new ConversationException("Invalid input to search user command.");
		}
		simulateHumanStyleMessageSending(channel, "Possible user matches:", session);
